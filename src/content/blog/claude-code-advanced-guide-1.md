...
1. `/pr-summary` 실행
2. `` !`gh pr diff` `` 등 셸 명령이 **먼저** 실행됨
3. 명령의 출력이 해당 위치에 텍스트로 치환됨
4. 치환된 전체 내용이 Claude에게 전달됨

즉 Claude가 실제로 보는 건 이런 형태다:

```markdown
## PR 컨텍스트
- PR diff: (실제 diff 출력 내용)
- PR 코멘트: (실제 코멘트 내용)
- 변경 파일: (실제 파일 목록)
...
`` !`command` ``는 **템플릿 변수**처럼 동작한다. 스킬 파일에 데이터를 직접 넣을 수 없으니, "실행 시점에 이 명령의 결과를 여기에 넣어라"는 뜻이다.
...
> **에이전트 타입이란?** Claude Code에는 용도별로 특화된 내장 에이전트 타입이 있다. `Explore`는 코드베이스 탐색 전용(읽기 전용, Haiku 모델), `Plan`은 계획 수립용, `general-purpose`는 범용(모든 도구 사용)이다. 커스텀 에이전트도 직접 만들 수 있다. 자세한 내용은 [Part 3](/blog/claude-code-advanced-guide-3)에서 다룬다.
...
Claude Code가 지원하는 모든 훅 이벤트다. `/hooks` 명령으로 확인할 수 있다.
...
| 이벤트 | 발생 시점 | 활용 예시 |
|---|---|---|
| `SubagentStart` | 서브에이전트가 시작될 때 | DB 연결 설정, 환경 준비 |
| `SubagentStop` | 서브에이전트가 완료될 때 | 리소스 정리, 결과 로깅 |

#### 에이전트 팀 이벤트

| 이벤트 | 발생 시점 | 활용 예시 |
|---|---|---|
| `TeammateIdle` | 팀원이 대기 상태에 진입할 때 | 피드백 전달로 계속 작업하게 유도 |
| `TaskCompleted` | 팀 태스크가 완료 처리될 때 | 완료 조건 검증, 품질 게이트 |
