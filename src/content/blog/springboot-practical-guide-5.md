...
- [왜 데이터베이스 최적화가 중요한가?](#1-왜-데이터베이스-최적화가-중요한가)
- [인덱스 (Index)](#2-인덱스-index)
- [페이지네이션 최적화](#3-페이지네이션-최적화)
- [Read Replica (읽기 복제본)](#4-read-replica-읽기-복제본)
- [N+1 문제](#5-n1-문제)
- [FAQ](#6-faq-면접-대비)
- [정리](#정리)
...
- [ ] 읽기 전용 트랜잭션에 `readOnly = true`가 설정되어 있는가?
- [ ] N+1 문제가 발생하는 쿼리가 없는가?
- [ ] 사용하지 않는 인덱스가 있지 않은가?
