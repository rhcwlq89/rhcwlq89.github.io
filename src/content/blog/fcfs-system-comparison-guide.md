...
**핵심 원리**: `DECR`은 지정한 키의 값을 1 감소시키는 Redis 명령어다. Redis는 모든 명령어를 싱글 스레드로 처리하므로 `DECR` 역시 원자적으로 실행되며, 별도의 락 없이도 안전하게 재고를 차감할 수 있다.
...
Redis는 `EVAL` 명령어를 통해 Lua 스크립트를 서버 내부에서 실행할 수 있다. 스크립트 안의 모든 명령어는 하나의 원자적 단위로 처리되므로, 여러 명령어를 묶어도 중간에 다른 요청이 끼어들지 않는다.

**핵심 원리**: 재고 확인과 차감을 하나의 Lua 스크립트로 묶어서 원자적으로 실행한다. 단순 DECR과 달리 **"확인 → 차감"이 한 덩어리**로 실행되어 음수 재고가 원천 차단된다.
...
**적합한 상황**: 수천~수만 명 동시 접속, 재고 확인 + 차감 + 중복 구매 방지를 DB까지 가지 않고 Redis 레벨에서 한 번에 처리해야 할 때. 단순 DECR로는 음수 재고 복구 로직이 부담되거나, 중복 체크를 별도 레이어에서 하기 싫은 경우에 적합하다.
...
| 단계 | 질문 | 예 | 아니오 |
|:---:|------|-----|--------|
| 1 | 동시 접속자가 수십 명 이하인가? | → **DB 비관적 락** (끝) | → 2번으로 |
| 2 | 추가 인프라(Redis) 도입이 가능한가? | → 3번으로 | → **DB 비관적 락** (끝) |
| 3 | 즉시 응답이 필수인가? | → 4번으로 | → 5번으로 |
| 4 | 중복 구매 방지까지 Redis에서 처리해야 하는가? | → **Redis + Lua** (끝) | → **Redis DECR** (끝) |
| 5 | 공정한 대기 순서(선착순 보장)가 중요한가? | → **대기열** (끝) | → **메시지 큐** (끝) |

> **토큰 발급**은 단독 방식이 아니라 봇 차단이나 트래픽 분리가 필요할 때 위 방식들과 **조합하는 보조 전략**이다. 예: 대기열 + 토큰, Redis + Lua + 토큰.
...
> 핵심은 <strong>"어떤 방식이 최고인가"가 아니라 "우리 상황에 뭐가 맞는가"</strong>다. 동시 접속 50명인 서비스에 Kafka를 도입하면 과잉 설계이고, 10만 명이 몰리는 티켓팅에 DB 락만 쓰면 서버가 다운된다.
