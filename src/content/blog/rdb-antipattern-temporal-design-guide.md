...
EAV가 **유일한 선택**인 경우도 있다:

- **속성이 수백 개이고 상품마다 다른 쇼핑몰**: 전자제품은 CPU/RAM/해상도, 의류는 소재/사이즈/시즌 — 컬럼으로 못 만든다
- **사용자 정의 필드**: SaaS에서 고객이 직접 필드를 추가하는 경우
- **설정 저장소**: key-value 형태가 자연스러운 경우

단, 이 경우에도 **핵심 속성은 컬럼으로, 가변 속성만 EAV로** 분리하는 것이 원칙이다.
...
- [ ] **EAV를 사용하고 있는가?** 핵심 속성은 컬럼으로, 가변 속성만 JSON으로 분리했는가?
...
1. **EAV는 유연해 보이지만 타입 안전성, 제약조건, 쿼리 성능을 전부 포기하는 패턴이다.** 핵심 속성은 컬럼으로, 가변 속성만 JSON으로 분리하는 것이 대안이다.
