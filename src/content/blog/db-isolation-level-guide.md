...
[MVCC 없이 (락 기반)]
TX1: UPDATE → 행 잠금 🔒
TX2: SELECT → 락 대기 ⏳ (TX1 끝날 때까지)
→ 읽기와 쓰기가 서로 차단

[MVCC 있을 때]
TX1: UPDATE → 새 버전(v2) 생성, 이전 버전(v1) 보관
TX2: SELECT → v1을 읽음 (락 없음, 즉시 실행)
→ 읽기와 쓰기가 서로 차단하지 않음
```
...
| 격리 수준 | MVCC 동작 |
|----------|----------|
| Read Uncommitted | 항상 최신 버전 (커밋 안 된 것도) |
| Read Committed | **각 쿼리 시작 시점**의 커밋된 최신 버전 |
| Repeatable Read | **트랜잭션 시작 시점**의 커밋된 버전 (스냅샷 고정) |
| Serializable | DB마다 다름 (MySQL: 락 추가, PostgreSQL: SSI) |

> 이 시리즈에서 "스냅샷을 읽는다", "undo log의 이전 버전" 같은 표현이 자주 나오는데, 모두 MVCC의 동작이다.
