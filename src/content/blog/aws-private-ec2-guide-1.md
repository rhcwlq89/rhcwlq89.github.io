...
"EC2는 Private Subnet에 두고, ALB와 NAT Gateway로 감싸세요"라는 말은 AWS를 조금만 검색해도 나온다. 하지만 **왜** 그래야 하는지 설명 없이 Terraform 코드부터 보여주는 글이 많다. 이 시리즈는 거기서 출발한다.

5편에 걸쳐 AWS에서 EC2를 Private Subnet에 두고, Bastion 없이 SSM으로 접속하고, GitHub Actions로 배포하고, 비용을 최적화하는 실무 가이드를 다룬다. 1편은 **"왜"에 대한 이야기**다 — 구성을 납득하고 2편으로 넘어가기 위한 토대다.

- **1편 — 왜 Private Subnet인가? (이 글)**
- 2편 — Terraform으로 VPC 인프라 구성하기
- 3편 — SSM Session Manager로 Bastion 없이 접속하기
- 4편 — GitHub Actions + SSM/CodeDeploy CI/CD 파이프라인
- 5편 — 비용 분석과 최적화 전략

이 글의 대상 독자는 "AWS 튜토리얼을 따라 EC2를 만들어봤지만 Private Subnet이나 NAT Gateway가 왜 필요한지는 모르겠는" 주니어다. 다 읽고 나면 **"아, 그래서 이렇게 해야 하는구나"**가 남아야 한다.
...
- **EC2는 Private Subnet에 둔다.** 공인 IP가 붙지 않고, 외부에서 직접 접근이 불가능하다. 인바운드 트래픽은 ALB를 통해서만 들어온다.
- **ALB는 Public Subnet에 둔다.** 인터넷에서 들어오는 HTTP/HTTPS 트래픽을 받아서 뒤의 Private EC2로 라우팅한다. EC2의 "대문" 역할.
- **NAT Gateway도 Public Subnet에 둔다.** EC2가 외부 API를 호출하거나, OS 패치를 받거나, 로그를 외부로 전송할 때 쓰는 **아웃바운드 전용 통로**다. 역방향(외부 → EC2) 접근은 불가능하다.
- **Multi-AZ 배치가 프로덕션 기준.** ALB, NAT Gateway, EC2 모두 2개 이상의 AZ에 나눠 둔다. 한 AZ가 장애나도 서비스는 살아있어야 하기 때문이다.

핵심 원칙 하나로 요약하면: **"인바운드는 ALB만, 아웃바운드는 NAT만, 그 외 직접 연결은 전부 차단"**.
...
**이 구성과의 관계**: Private Subnet의 EC2는 Public IPv4도 EIP도 없다. 외부 노출 자체가 없기 때문이다. 인바운드는 ALB가, 아웃바운드는 NAT Gateway가 처리한다. 이것이 Private Subnet이 보안상 유리한 이유 중 하나다.
...
하나라도 "중규모 이상" 쪽에 해당하면 Private Subnet 구성을 검토할 시점이다. 특히 **컴플라이언스와 데이터 민감도**는 트래픽 규모와 상관없이 무조건 "중규모 기준"으로 가야 한다.

### 3.3 참고: HA(High Availability)란?

위 표에서 "EC2 2대 이상"과 "99.9%+ SLA"가 나왔다. 이건 HA와 직결된 얘기라 짧게 설명하고 넘어간다.

HA는 **"서비스가 죽지 않고 계속 살아있는 상태"**를 말한다. ALB는 HA를 달성하기 위한 도구 중 하나다.
...
> [!NOTE]
> **핵심 판단 기준**: Private Subnet 구성의 월 $60~140 추가 비용을 정당화할 수 있는가? 사이드 프로젝트나 초기 스타트업에서 월 $40이면 되는 걸 $140 쓰는 건 낭비고, 반대로 개인정보를 다루는데 비용 아끼려고 Public Subnet에 두는 건 리스크다. 상세 비용 분석은 5편에서 다룬다.
...
- Public Subnet의 EC2는 공인 IP가 할당되어 **인터넷에서 직접 접근 가능**하다.
- Security Group 설정 실수 하나로 DB 포트(3306, 5432 등)나 SSH(22)가 전세계에 열릴 수 있다. 실제로 이런 설정 실수로 인한 사고가 매년 발생한다.
- 공인 IP 자체가 봇/스캐너의 **자동 탐색 대상**이다. AWS에 EC2를 띄우는 순간 몇 분 안에 22번 포트 브루트포스 시도가 로그에 찍히기 시작한다.
- 개인정보가 있는 서버가 직접 노출되면, 침투 시 **바로 데이터 유출**로 이어진다. 공격자 입장에서 "웹 서버 → 내부 망 → DB"라는 단계를 건너뛸 수 있다.
- Private Subnet 구성이라면 ALB를 통해서만 인바운드가 들어오므로 **방어 레이어가 하나 더 생긴다**. ALB가 L7에서 비정상 요청을 차단할 수 있고, WAF까지 붙이면 공격 탐지/차단이 가능하다.

### 4.2 컴플라이언스 위반

- **개인정보보호법, ISMS-P 등의 규제에서 네트워크 분리(망분리) 요건**이 존재한다. "개인정보 처리 시스템은 외부 네트워크와 물리적/논리적으로 분리해야 한다"는 조항이 있다.
- 감사 시 "개인정보 처리 서버가 Public Subnet에 있고 공인 IP로 직접 노출되어 있다"는 것 자체가 **지적 사항**이 된다. 기술적 안전조치 미흡으로 판단될 수 있다.
- 사고 발생 시 "합리적 보호 조치를 하지 않았다"는 **과실 인정 근거**가 된다. 법적 책임의 범위가 넓어진다.

### 4.3 사고 시 책임 확대

- Private Subnet + ALB 구성이면 사고 후 감사에서 **"표준 보안 아키텍처를 적용했다"**고 항변할 수 있다. AWS Well-Architected Framework의 보안 권고를 따랐다는 근거가 된다.
- Public Subnet에 방치했다면 **"비용 절감을 위해 보안을 소홀히 했다"**는 판단을 받을 수 있다.
- 이 판단이 **과징금 규모와 손해배상 판결에 직접 영향**을 준다. 과실 범위가 커질수록 배상액도 커진다.

> [!NOTE]
> **결론**: 공개 데이터만 다루는 사이드 프로젝트라면 Public Subnet + Security Group으로 충분하다. 하지만 개인정보(회원 데이터, 결제 정보, 민감 정보)가 들어가는 순간 네트워크 레벨 격리(Private Subnet)는 **비용이 아니라 보험**이다. 월 $60~100의 추가 비용과 사고 시 과징금·평판 손해를 비교하면 방향은 명확하다.

실무적으로는 **소규모에는 Public Subnet + SG로 시작하고, 규모가 커지거나 개인정보를 다루게 되면 Private Subnet 구조로 마이그레이션**하는 게 현실적이다. 처음부터 무조건 풀 구성을 잡을 필요는 없지만, 데이터 성격이 바뀌는 순간 주저 없이 넘어가야 한다.
...
1. **표준 아키텍처는 "ALB + Private EC2 + NAT Gateway"** 다. 인바운드는 ALB만, 아웃바운드는 NAT만, 그 외 직접 연결은 차단하는 원칙이다.
2. **Public IPv4와 EIP의 차이**를 알아야 왜 Private Subnet 구성에서 둘 다 필요 없는지 이해된다. 외부 노출 자체가 없기 때문이다.
3. **모든 서비스가 이 구성을 써야 하는 건 아니다.** 소규모 사이드 프로젝트에는 월 $40짜리 Public Subnet + SG 구성이 합리적이고, 중규모 이상이거나 컴플라이언스 요구가 있을 때 Private Subnet으로 넘어가면 된다.
4. **HA는 "EC2 2대 + ALB + Multi-AZ"** 로 달성한다. 이게 Private Subnet 구성이 필요해지는 실질적인 전환점 중 하나다.
5. **개인정보 + Public Subnet의 3가지 리스크**: 직접 공격 노출, 컴플라이언스 위반, 사고 시 책임 확대. 개인정보를 다루는 순간 Private Subnet은 비용이 아니라 보험이다.

1편의 목표는 하나였다 — **구성을 납득하는 것**. "왜 Private Subnet인가"에 대해 "어? 그럼 이렇게 해야겠네" 소리가 나오면 됐다. 2편부터는 이 아키텍처를 실제 코드로 만든다.

다음 편에서는 **Terraform으로 VPC 인프라 구성하기** — VPC CIDR 설계부터 2AZ Public/Private Subnet, Route Table, Security Group의 "SG-참조 패턴", ALB와 EC2까지 단일 `main.tf`로 엮어서 `terraform apply` 한 번으로 올리는 과정을 다룬다.
