...
앞에서 "Multi-AZ 배치가 프로덕션 기준"이라고 했는데, 실제로 어떻게 구성되는지 짚고 넘어간다. 먼저 Subnet · AZ · VPC의 관계부터.

<strong>핵심 사실 3가지</strong>:

1. <strong>Subnet은 정확히 하나의 AZ에만 속한다.</strong> Subnet을 만들 때 "어느 AZ에 만들지"를 반드시 지정해야 한다. 한 Subnet 안에 여러 AZ의 EC2를 넣을 수는 없다.
2. <strong>VPC는 리전 전체를 덮는다.</strong> 같은 VPC 안에서 AZ별로 Subnet을 여러 개 만들면, 자연스럽게 Multi-AZ 구성이 된다.
3. <strong>ALB 하나가 여러 AZ를 걸쳐서 동작한다.</strong> ALB를 만들 때 AZ별 Public Subnet을 여러 개 attach하면 AWS가 내부적으로 cross-AZ 라우팅을 자동 처리한다. <strong>ALB를 AZ마다 따로 만들면 안 된다</strong> — DNS 엔드포인트가 둘로 쪼개지고, ALB 자체의 HA도 못 쓰게 된다.

<strong>2AZ Multi-AZ 실제 구조</strong>:
...
- VPC는 1개 (리전 전체)
- ALB는 1개 (양쪽 Public Subnet에 attach돼서 두 AZ 모두 바라봄)
- Public Subnet은 AZ마다 1개 → 총 2개
- Private Subnet도 AZ마다 1개 → 총 2개
- NAT Gateway는 AZ마다 1개 → 총 2개 (AZ 장애 격리를 위해. 비용 최적화를 위해 단일 AZ NAT로 가는 선택지도 있음 — 5편에서 다룬다)
- EC2는 각 Private Subnet에 분산 배치

<strong>한 AZ가 죽으면 어떻게 되나</strong>: AZ-a 전체가 장애나도 AZ-c의 Public/Private Subnet과 EC2는 그대로 살아있다. ALB는 살아있는 AZ-c로만 트래픽을 보내고, 사용자는 장애를 거의 느끼지 못한다. 이것이 "Multi-AZ로 HA를 달성한다"의 실제 의미다.

<strong>왜 ALB는 AZ마다 따로 만들면 안 되는가</strong>:

- ALB 하나당 DNS 엔드포인트 하나다. 2개 만들면 Route 53에서 어느 것을 줄지 직접 관리해야 한다 (weight / failover 라우팅 설정).
- AWS가 제공하는 ALB 자체의 AZ 간 가용성을 못 쓰게 된다. ALB는 내부적으로 이미 Multi-AZ다.
- 비용 2배, 운영 복잡도 2배.

---

### 1.4 이 시리즈에서 자주 나오는 약어
