...
- <strong>측정하려면 DORA를 본다</strong> — 배포 빈도·변경 리드타임·변경 실패율·복구 시간. 내부 점수(벨로시티)가 아니라 전달 성과(outcome)를 잰다. 속도 ≠ 가치.
...
| 단순 설계(YAGNI) | 지금 필요한 것만 만든다 | 복잡도 누적 방지(1편 10원칙) |
...
벨로시티가 내부 점수라면, <strong>DORA 4 메트릭</strong>은 전달 성과(outcome)를 잰다. 'Accelerate' 연구에서 정리된, 소프트웨어 전달 성과를 대표하는 네 지표다.
...
| 복구 시간(MTTR) | 장애에서 회복까지 걸리는 시간 | 안정성 |
...
| DORA 4 메트릭 | 배포 빈도·변경 리드타임·변경 실패율·복구 시간 — 소프트웨어 전달 성과 지표 |
