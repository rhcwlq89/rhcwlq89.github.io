...
> **실무 팁**: 대부분 `BIGINT AUTO_INCREMENT` 단일 PK를 쓰기 때문에 2NF 위반은 잘 발생하지 않는다. <strong>복합키를 쓰는 중간 테이블(N:M 관계)</strong>에서만 주의하면 된다.
...
3NF까지 했는데도 문제가 남는 경우가 있다. <strong>"PK가 아닌 컬럼이 다른 컬럼을 결정하는 경우"</strong>다.
