...
SAML에서 SSO 방식은 크게 **Active**(기본값)와 **Passive**(IsPassive=true) 두 가지로 나뉜다.

| 구분 | Active SSO | Passive SSO |
|------|-----------|-------------|
| 사용자 인터랙션 | 허용 (필요 시 로그인 UI 표시) | 금지 (UI 표시 불가) |
| IdP 세션 없을 때 | 로그인 화면 → 인증 완료 후 SP 복귀 | `NoPassive` 오류 또는 SP가 직접 처리 |
| IdP 세션 있을 때 | 자동 처리 (ForceAuthn=false 조건) | 자동 처리 |
| AuthnRequest 속성 | `IsPassive="false"` (기본값) | `IsPassive="true"` |
| 사용 시나리오 | SSO 버튼 클릭, 보호된 리소스 접근 | 페이지 진입 시 자동 로그인 시도 |
| 실패 처리 | IdP가 로그인 화면 표시 | SP가 직접 처리 (예: 로그인 버튼 노출) |

> **핵심**: Passive SSO는 "로그인되어 있으면 가져오고, 아니면 말고"라는 전략이다. 사용자를 로그인 화면으로 강제로 보내지 않기 때문에 UX가 부드럽다. Active SSO는 "반드시 인증된 상태를 보장해야 한다"는 전략이다.
